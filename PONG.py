...
def Pong():
    ...
    # Schläger
    # Starthöhen Schläger
    Hoehe_S1 = BOARD_HEIGHT / 5 * 2
    Hoehe_S2 = BOARD_HEIGHT / 5 * 2
    ...
    def easteregg():
        if Resultat[0] == 0 and Resultat[1] == 10 or Resultat[0] == 10 and Resultat[1] == 0:
            font = pygame.font.SysFont("comicsansms", 120)
            text = font.render(("Haha, Versager!!!"), True, (RED))
    ...
    while status == "Start":
        ...
        font = pygame.font.SysFont("comicsansms", 72)
        text = font.render(("Wähle den Modus"), True, (WHITE))
    ...
    while status == "PvP" or status == "Easy" or status == "Hard" or status == "Destroyer":
        ...
        if Resultat[0] >= 10 or Resultat[1] >= 10:
            status = "Ende"
    ...
    while status == "Ende":
        ...
        if button(BOARD_LENGTH/2 - 150, BOARD_HEIGHT/5*3, 300, 70, WHITE, "Noch einmal?", 50):
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    Pong()
        if button(BOARD_LENGTH/2 - 150, BOARD_HEIGHT/5*4, 300, 70, WHITE, "Schliessen", 50):
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    pygame.quit()
                    sys.exit()
